...
class Device():
    ...
    def adb_input(self, text):
        adb_exe(
            'adb -s %s shell am broadcast -a ADB_INPUT_TEXT --es msg "%s"' % (self.s, text))

    # 封装swipe
    def adb_swipe(self, y0, y1):
        adb_exe('adb -s %s shell input swipe 240 %s 240 %s ' %
                (self.s, y0, y1))
    ...
    def simu_loc(self, lat, long):
        ...
        # 打开天下游
        adb_exe(
            'adb -s %s shell am start com.txy.anywhere/com.txy.anywhere.activity.SplashActivity' % self.s)
        # 点击输入经纬度信息
    ...
    def add_before(self):
        ''' 进入微信添加好友界面 '''
        adb_exe(
            'adb -s %s shell am start com.tencent.mm/com.tencent.mm.ui.LauncherUI' % self.s)
        # 发现
    ...
    def get_add_btn_pos(self):
        ''' 获得打招呼按钮的btn '''
        adb_exe('adb -s %s shell screencap -p /sdcard/tmp%s.png' %
                (self.s, self.s))
